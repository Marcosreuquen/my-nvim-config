    ...
    config = function()
      ...
      local function ask_temp()
        ...
        Snacks.input({ prompt = "Ask AI: " }, function(question)
          ...
          -- NOTE: It should select the agent from 'opencode models | grep opencode/', using first: 'opencode/gpt-5-nano', if it's not available, use any other from the opencode models | grep opencode/ list.
          -- local opencode_models = vim.fn.system({ "opencode", "models", "--format", "json" })
          local default_model = "opencode/gpt-5-nano"

          local run_cmd = { "opencode", "run", "--title", "temp", "--model", default_model, "--agent", "quick-question", "--port", "54404" } -- It depends on having the agent called "quick-question"
